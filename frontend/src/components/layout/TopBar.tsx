    ...
    <div className="flex items-center gap-2 sm:gap-4 relative z-50">
      ...
      {!isHOD && (
        <div ref={notifRef} className="relative z-50">
          ...
          {isNotifOpen && (
            <div className="absolute right-0 top-full mt-2 w-72 sm:w-96 glass-card p-4 rounded-2xl border border-cyan-500/50 shadow-2xl z-[9999] bg-surface-900/95 backdrop-blur-2xl animate-slide-up space-y-3">
      ...
      <span className="text-xs sm:text-sm font-medium text-slate-300">
        <span className="hidden xs:inline">@{user?.username || 'User'}</span>{' '}
        <span className="text-[10px] sm:text-xs bg-cyan-500/20 text-cyan-400 px-2 py-0.5 rounded-full font-bold">
          {user?.role}
        </span>
      </span>
      
      <button onClick={handleLogout} className="text-xs sm:text-sm text-red-400 hover:text-red-300 font-semibold transition-colors cursor-pointer">
        Logout
      </button>
    </div>
  );
